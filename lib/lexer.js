...
function lex(_input) {
	...
	const pushToken = (t, c, i) => {
		const _push = (token, _type, opt) => {
			...
			if (type == "string")
				index--;
			else if (type == "type" && !(["comment", "reference", "required", "const", "exprDelim"].includes(lastToken.type)))
				warn("unexpected type after " + lastToken.type, index)
	...
	for (let i = 0; i < input.length; i++) {
		...
		// comment mode
			...
			if (!(["comment", "bracketOpen", "argDelim", "exprDelim", "return", "break", "pipe", "operator", "reference", "string"].includes(lastToken.type)))
				warn("did not expect string after " + lastToken.type, i);
			...
			if (c != "!" && !(lastToken.type == "operator" && lastToken._ == "!") &&
			   !(["comment", "exprDelim", "pipe", "bracketClose", "required", "const", "true", "false", "default", "number", "string", "reference"].includes(lastToken.type)))
				warn("unexpected operator after " + lastToken.type, i);
