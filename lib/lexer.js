...
export function lex(_input) {
	...
	for (let i = 0; i < input.length; i++) {
		...
		// comment mode
			...
			if (temp != "")
				pushToken("?", c, i);
			if (!(["comment", "bracketOpen", "argDelim", "exprDelim", "operator"].includes(lastToken.type)))
			...
			if (temp != "")
				pushToken("?", c, i);
			if (c != "!" && !(["comment", "exprDelim", "bracketClose", "reference", "number", "string"].includes(lastToken.type)))
